...
function getRequestOrigin(req: Request): string {
  ...
  if (clientOrigin && (clientOrigin.startsWith('http://') || clientOrigin.startsWith('https://'))) {
    ...
    return clientOrigin.replace(/\/+$/, '').replace('0.0.0.0', 'localhost');
  }

  if (req.headers.origin && typeof req.headers.origin === 'string') {
    return req.headers.origin.replace(/\/+$/, '').replace('0.0.0.0', 'localhost');
  }

  if (req.headers.referer && typeof req.headers.referer === 'string') {
    try {
      const parsed = new URL(req.headers.referer);
      return parsed.origin.replace(/\/+$/, '').replace('0.0.0.0', 'localhost');
    } catch {}
  }

  // For x-forwarded headers (Vercel/production)
  const forwardedProto = req.headers['x-forwarded-proto'] as string;
  const forwardedHost = req.headers['x-forwarded-host'] as string;
  if (forwardedProto && forwardedHost && !forwardedHost.includes('0.0.0.0')) {
    return `${forwardedProto}://${forwardedHost}`.replace(/\/+$/, '');
  }

  // Local host header fallback — use http for localhost
  const host = req.headers.host;
  if (host) {
    const normalizedHost = host.replace('0.0.0.0', 'localhost');
    const isLocal = normalizedHost.includes('localhost') || normalizedHost.includes('127.0.0.1');
    const proto = isLocal ? 'http' : (forwardedProto || 'https');
    return `${proto}://${normalizedHost}`.replace(/\/+$/, '');
  }

  // Final fallback: APP_URL from env
